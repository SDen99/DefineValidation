...
<script lang="ts">
	...
	import { Card, CardContent, CardHeader, CardTitle } from '@sden99/ui-components';
	import { Badge } from '@sden99/ui-components';
	import { GripVertical, Eye, EyeOff } from '@lucide/svelte/icons';
...
<Card class="w-full">
	<CardHeader>
		<CardTitle class="text-sm">Variables & Visibility</CardTitle>
	</CardHeader>
		...
		<div class="max-h-96 space-y-1 overflow-y-auto">
			{#each columnInfo as column}
				...
				>
					...
					{:else}
						<div class="w-4"></div>
						<!-- Spacer for hidden columns -->
					{/if}

					<!-- ✅ FIXED: Changed onchange to onCheckedChange -->
					<Checkbox
					...
					<!-- Visibility icon -->
					<IconComponent class="h-4 w-4 {column.visible ? 'text-success' : 'text-muted-foreground'}" />
					...
					<div class="min-w-0 flex-1">
						...
						{#if column.visible && column.order >= 0}
							<span class="text-xs text-muted-foreground">Position: {column.order + 1}</span>
						{/if}
					</div>
				</div>
			{/each}
