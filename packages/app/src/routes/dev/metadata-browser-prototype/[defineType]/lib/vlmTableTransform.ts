...
export function transformVLMForEditing(
	...
	console.log(
		`[vlmTableTransform] Total variables: ${allVariables.length}, VLM variables: ${vlmVariables.length}`
	);

	// DEBUG: Expose raw data
	if (typeof window !== 'undefined') {
		(window as any).$temp_allVariables = allVariables;
		(window as any).$temp_vlmVariables = vlmVariables;
	}
	...
	// DEBUG: Log detected stratification hierarchy
	console.log('[vlmTableTransform] Stratification Hierarchy Detected:', {
		primary: hierarchy.primary,
		secondary: hierarchy.secondary,
		analysis: hierarchy.analysis,
		totalVLMVariables: vlmVariables.length
	});
	...
	console.log(
		`[vlmTableTransform] Generated ${vlmTableData.rows.length} rows, ${vlmTableData.columns.length} columns`
	);
	console.log('[vlmTableTransform] Stratification Columns:', Array.from(vlmTableData.stratificationColumns));
	console.log('[vlmTableTransform] Sample Row:', vlmTableData.rows[0]);
	...
	const allItemDefs = itemGroup?.itemRefs?.map((ref: any) => {
		const itemDef = targetDataset.enhancedDefineXML?.lookups?.itemDefsByOID?.get(ref.itemOID);
		return itemDef;
	}).filter(Boolean) || [];
	...
	// DEBUG: Expose final data
	if (typeof window !== 'undefined') {
		(window as any).$temp_vlmProcessingOutput = vlmTableData;
		(window as any).$temp_parameterGroups = parameterGroups;
	}
