<script lang="ts">
	...
	const codelist = $derived(
		activeData.defineData.CodeLists?.find((cl) => cl.OID === $page.params.oid)
	);
	...
	const usedByVariables = $derived.by(() => {
		const variables = activeData.defineData.ItemDefs?.filter((item) => {
			// Exclude variables that are marked as deleted
			if (item.OID) {
				const change = editState.getChange(defineType, 'variables', item.OID);
		...
		return variables.flatMap((variable) => {
			const datasetsUsingVariable = activeData.defineData.ItemGroups?.filter((ig) => {
				...
				if (ig.OID) {
					const change = editState.getChange(defineType, 'datasets', ig.OID);
					if (change?.type === 'DELETED') return false;
				}
	...
	// Merge CodeListItems and EnumeratedItems for display
	// EnumeratedItems don't have a separate Decode element - CodedValue is both code and display
	const allCodeListItems = $derived.by(() => {
		const codeListItems = editableCodelist?.CodeListItems || [];
		const enumeratedItems = (editableCodelist?.EnumeratedItems || []).map((item) => ({
			...item,
			Decode: null, // EnumeratedItems don't have Decode
			Rank: null,
			ExtendedValue: false
		}));
	...
	function addCodedValue() {
		...
		const items = editableCodelist.CodeListItems || [];
		const nextOrderNumber = allCodeListItems.length + 1;

		const newItem: CodeListItem = {
			CodedValue: '',
			OrderNumber: nextOrderNumber.toString(),
			Rank: null,
			ExtendedValue: !!editableCodelist.StandardOID, // Auto-set for standard codelists
			Decode: {
				TranslatedText: '',
				Lang: 'en'
			},
	...
	function deleteCodedValue(index: number) {
		...
		// Confirm deletion
		if (!confirm(`Delete coded value "${items[index].CodedValue}"?`)) {
			return;
		}
...
{#if codelist && editableCodelist}
	<div class="mx-auto max-w-4xl">
		...
		<div class="mb-6">
			...
			{:else}
				<h1 class="mb-2 text-3xl font-bold">{editableCodelist.Name || codelist.OID}</h1>
			{/if}
		...
		<div class="mb-6 rounded-lg border bg-card">
			<div class="border-b p-4 flex items-center justify-between">
				<h2 class="text-lg font-semibold">
					Coded Values ({allCodeListItems.length})
				</h2>
				{#if editState.editMode && !isAlreadyDeleted}
					<button
								...
								<div class="flex items-start gap-4 rounded-md border p-3">
									...
									<div class="flex-1">
										<div class="font-medium">
											{item.Decode?.TranslatedText || item.CodedValue}
										</div>
