<script lang="ts">
	...
	const whereclause = $derived(
		activeData.defineData.WhereClauseDefs?.find((wc) => wc.OID === $page.params.oid)
	);

	// Find associated comment
	const comment = $derived(
		whereclause?.CommentOID
			? activeData.defineData.Comments?.find((c) => c.OID === whereclause.CommentOID)
			: null
	);
	...
	const usedByVariables = $derived.by(() => {
		...
		activeData.defineData.ValueListDefs?.forEach((vl) => {
	...
	function getItemDefName(oid: string): string {
		const itemDef = activeData.defineData.ItemDefs?.find((item) => item.OID === oid);
