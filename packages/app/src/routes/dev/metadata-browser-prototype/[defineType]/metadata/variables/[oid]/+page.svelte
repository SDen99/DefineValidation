<script lang="ts">
	...
	const variable = $derived(
		activeData.defineData.ItemDefs?.find((item) => item.OID === $page.params.oid)
	);
	...
	const { navigateToCodeList, navigateToDataset } = createNavigationHandlers($page.url.pathname);
	...
	const oidVariants = $derived.by(() => {
		...
		return activeData.defineData.ItemDefs?.filter((item: any) =>
	...
	const usedByDatasets = $derived.by(() => {
		...
		const datasets = activeData.defineData.ItemGroups?.filter((ig) => {
	...
	const referencedCodeList = $derived.by(() => {
		...
		const codelist = activeData.defineData.CodeLists?.find((cl) => cl.OID === editableVariable.CodeListOID);
		if (!codelist?.OID) return null;

		// Exclude codelists that are marked as deleted
		const change = editState.getChange(defineType, 'codelists', codelist.OID);
	...
	const referencedValueList = $derived.by(() => {
		...
		return activeData.defineData.ValueLists?.find((vl) => vl.OID === editableVariable.ValueListOID);
	});
		...
		<div class="mb-6 rounded-lg border bg-card">
			...
			<div class="p-4">
				<dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
					...
					<!-- SASFieldName -->
					{#if editableVariable.SASFieldName || editState.editMode}
						<div>
					...
					<!-- SignificantDigits -->
					{#if editableVariable.SignificantDigits || editState.editMode}
						<div>
					...
					<!-- DisplayFormat -->
					{#if editableVariable.DisplayFormat || editState.editMode}
						<div>
				...
				{#if editableVariable.Description || editState.editMode}
					<div class="mt-4">
							...
							>
								<div class="flex items-center justify-between gap-4">
									<div class="flex-1">
										...
										{#if dataset.Label}
											<div class="text-xs text-muted-foreground">{dataset.Label}</div>
										{/if}
									</div>
