<script lang="ts">
	...
	const method = $derived(
		activeData.defineData.Methods?.find((m) => m.OID === $page.params.oid)
	);
	...
	const usedByVariables = $derived.by(() => {
		...
		activeData.defineData.ItemGroups?.forEach((ig) => {
...
{#if method}
	<div class="mx-auto max-w-4xl">
		...
		{#if editState.editMode || editableMethod?.Document || editableMethod?.Pages || editableMethod?.TranslatedText}
			<div class="mb-6 rounded-lg border bg-card p-4">
